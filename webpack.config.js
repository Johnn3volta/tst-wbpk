...
module.exports = (env) => {
  /* присваиваем зависимость dev в зависимости от запущенной клманды.
   * Проставляется в package.json scripts --env  */
  ...
  const conf = {
    ...
    module: {
      ...
      rules: [
        ...
        *  postcss-loader - делает обработку css в соответсвии с параметрами
        *  задаными в файле postcss.config.js
        *  sass-loader    - компилирует css
        *
        *  (?sourceMap делает подключает sourceMap в соответсвии со значением
        *  с devtool) */
        {
          test: /\.(sc|c)ss$/,
          use: [
        ...
        // отвечает за файлы изображений.
        {
          ...
          use: {
            ...
            loader: 'url-loader',
            options: {
              // если размер файла меньше указаного лимита картинка из стилей
              // подключается в base64
              limit: 8000,
    ...
    plugins: [
      ...
      new HtmlWebpackPlugin({
        ...
        filename: 'index.html',
        /* Название точки входа для подтягивания css и js. С свйойством inject:
        * false скрипты и стили подключаем вручну в исходном файле */
        chunks: [
          'main',
        ],
      }),
